...
<h2><?php  __('Newsletter');?></h2>
	<dl><?php $i = 0; $class = ' class="altrow"';?>
		...
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
                        <?php if(!empty($news['News']['attachment']))
                            echo $html->link($newsletter['Newsletter']['attchment'], "/files/attachments/".$newsletter['Newsletter']['attchment']);
                            else
                                echo 'no';
                        ?>
		</dd>
		...
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $html->link($newsletter['User']['name'], array('controller'=> 'users', 'action'=>'view', $newsletter['User']['id'])); ?>
			&nbsp;
		</dd>
	</dl>
</div>
<div class="actions">
	<ul>
		...
		<li><?php echo $html->link(__('Delete', true), array('action'=>'delete', $newsletter['Newsletter']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $newsletter['Newsletter']['id'])); ?> </li>
                <li><?php echo $html->link(__('Spedisci', true), array('action'=>'deliver', $newsletter['Newsletter']['id'])); ?> </li>
